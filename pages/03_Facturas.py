...
with tabs[2]:
    ...
    with st.container(border=True):
        c1_fact, c2_fact, c3_fact, c4_fact = st.columns([2.2, 1.3, 1.3, 1.2])

        with c1_fact:
            rfc_in_fact = st.text_input("RFC", value=last_rfc_fact or "", placeholder="PEIC211118IS0", key="facturas_rfc")
        ...
        with c4_fact:
            run_fact = st.button("Cargar", type="primary", use_container_width=True, disabled=not (12 <= len(rfc_fact) <= 13), key="btn_cargar_facturas")

    if run_fact:
        with st.spinner("Consultando facturas en Syntage…"):
            local_dir_fact = str(ROOT / "data" / "cfdi_xml")
            try:
                cfdi_data_fact = fetch_cfdi(rfc=rfc_fact, source=cfdi_source, date_from=d_from_fact, date_to=d_to_fact, local_dir=local_dir_fact)
                st.session_state["cfdi_data"] = cfdi_data_fact
                st.session_state["last_rfc"] = rfc_fact
                st.session_state["cfdi_date_from"] = d_from_fact
                st.session_state["cfdi_date_to"] = d_to_fact
            except Exception as e:
